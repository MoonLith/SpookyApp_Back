...
export const getPlantHistoryById = async  (req : Request, res : Response) =>{
    try{
...
const execute = async (email :string, username :string, deviceId :string): Promise<any> => {

    const currentUser = await UserRepository.findByUsername(username);

    checkThatUserExistsOrThrow(currentUser);

    const result =  await PlantHistoryRepository.findById(deviceId);

    // return plantHistoryToResponse(result['Items']);
    return convertPlantHistory(result['Items'][0]);
